...
clipboardResultElems.forEach((elem) => {
  ...
  errorSlot.appendChild(
    new Text(
      "Could not read from the clipboard. Please grant permission for Copy Hero to read your clipboard in order to enable the JSON conversion shortcut. If you have just granted permission to Copy Hero to read from clipboard, please try again."
    )
  );
